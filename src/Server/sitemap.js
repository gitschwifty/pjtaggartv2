...
dbAPI
    ...
    .then(function (posts) {
    var noActifitPosts = posts.filter(function (post) { return post.category !== 'actifit' && post.author === 'petertag'; });
    noActifitPosts.map(function (post) { return ({
    ...
    var sitemap = sm.createSitemap({
        ...
        ].concat(noActifitPosts.map(function (post) { return ({
            url: hostname + '/post/' + post.permlink,
            changefreq: 'monthly',
            priority: 0.3
        }); }))
